...
- **R26** — "Cada sorteio só deve ser realizado ao selecionar a tecla enter" (`enunciado.txt:63`). É a posição do único `cin.ignore` do laço (`85`), entre mostrar o número e voltar ao topo: **nenhum número sai sem um Enter**. A ressalva honesta, que vale ensaiar numa frase só: o sorteio acontece no topo da volta seguinte, não no instante do toque — o Enter confirma o número da vez e libera o próximo. Ver `docs/91-o-que-pode-ser-contestado.md`, item C4.
...
- **Nome comprido pode invadir a cartela vizinha.** O `docs/99-o-que-foi-medido.md` mediu que a Versão Final ocupa 115 colunas × 26 linhas com os nomes da equipe — cabe em 120×30, com folga pequena. As cartelas 1 e 2 ficam a 40 colunas de distância (`x1=2`, `x2=42`, `jogo.cpp:29-30`), e `exibir` escreve `"Jogador: " + nome` sem cortar (`exibicao.cpp:57-58`). Um nome longo o bastante invade visualmente a área da cartela ao lado até ser sobrescrito por ela, já que `exibir(mat2,...)` roda logo em seguida (`jogo.cpp:61`). Não testei ao vivo com um nome desse tamanho.
